...
function clearRequests () {
  ...
  if (xhr) {
    for (var i = 0; i < xhr.length; i++) {
      try { xhr[i].onprogress = null; xhr[i].onload = null; xhr[i].onerror = null; } catch (e) { console.error(e); }
      try { xhr[i].upload.onprogress = null; xhr[i].upload.onload = null; xhr[i].upload.onerror = null; } catch (e) { console.error(e); }
      try { xhr[i].abort(); } catch (e) { console.error(e); }
      try { delete (xhr[i]); } catch (e) { console.error(e); }
    }
    xhr = null;
  }
}
  ...
  var testStream = function (i, delay) {
    setTimeout(function () {
      ...
      xhr[i].onprogress = function (event) {
        ...
        if (testStatus !== 1) { try { x.abort(); } catch (e) { console.error(e); } } // just in case this XHR is still running after the download test
      ...
      xhr[i].onload = function () {
        ...
        try { xhr[i].abort(); } catch (e) { console.error(e); } // reset the stream data to empty ram
        testStream(i, 0);
      }.bind(this);
      ...
      // send xhr
      try { if (settings.xhr_dlUseBlob) xhr[i].responseType = 'blob'; else xhr[i].responseType = 'arraybuffer'; } catch (e) { console.error(e); }
...
// garbage data for upload test
  ...
  try { r = new Uint32Array(r); for (let i = 0; i < r.length; i++)r[i] = Math.random()*maxInt; } catch (e) { console.error(e); }
  var req = [];
  var reqsmall = [];
    ...
    setTimeout(function () {
      ...
      if (ie11workaround) {
        ...
        xhr[i].upload.onprogress = function (event) {
          ...
          if (testStatus !== 3) { try { x.abort(); } catch (e) { console.error(e); } } // just in case this XHR is still running after the upload test
          // progress event, add number of new loaded bytes to totLoaded
          var loadDiff = event.loaded <= 0 ? 0 : (event.loaded - prevLoaded);
          if (isNaN(loadDiff) || !isFinite(loadDiff) || loadDiff < 0) return; // just in case
          totLoaded += loadDiff;
